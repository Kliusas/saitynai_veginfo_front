    ...
    >
      <div class="mb-3 col-12" key="deleteBlock" v-if="$cookies.get('auth')">
        <portlet horizontal-full>
          <template slot="title"
            >Want to add new Event Category?</template
          >
...
export default {
  ...
  methods: {
    deleteItem(id) {
      this.$http.delete(`http://localhost/event-categories/${id}`).then(() => {
    ...
    updateData(item) {
      this.$http.patch(`http://localhost/event-categories/${item.id}`, {
        name: item.name
      });
    ...
    submitData(item) {
      ...
      this.$http
        .post("http://localhost/event-categories", {
          data: { name: item.name }
        })
        .finally(() => {
          item.name = "";
          this.$http.get("http://localhost/event-categories").then(response => {
