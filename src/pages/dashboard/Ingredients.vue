    ...
    >
      <div class="mb-3 col-12" key="deleteBlock " v-if="$cookies.get('auth')">
        <portlet horizontal-full>
          <template slot="title"
            >Want to add new Ingredient?</template
          >
...
export default {
  ...
  methods: {
    deleteItem(id) {
      this.$http.delete(`http://localhost/ingredients/${id}`).then(() => {
    ...
    updateData(item) {
      this.$http.patch(`http://localhost/ingredients/${item.id}`, {
        name: item.name
      });
    ...
    submitData(item) {
      ...
      this.$http
        ...
        .finally(() => {
          item.name = "";
          this.$http.get("http://localhost/ingredients").then(response => {
